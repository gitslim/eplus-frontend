...
<script>
    ...
    const heroSetSpeed = (s) => {
        heroSpeed = s
        heroEl.playbackRate = s
        heroSpeedMenu = false
    }
    const heroFullscreen = () => {
        if (!document.fullscreenElement) {
            heroEl.closest(".video-box").requestFullscreen()
        } else {
    ...
    const bmkSetSpeed = (s) => {
        bmkSpeed = s
        bmkEl.playbackRate = s
        bmkSpeedMenu = false
    }
    const bmkFullscreen = () => {
        if (!document.fullscreenElement) bmkEl.closest(".bmk-video-box").requestFullscreen()
    ...
    const schemeItems = [
        {
            ...
            link: "https://энергияплюс.рф/page/funkcii-generalnogo-proektirovshika",
            arrowDeg: 75,
        },
        {
            ...
            link: "https://энергияплюс.рф/page/funkcii-tehnicheskogo-zakazchika",
            arrowDeg: 135,
        },
        {
            ...
            midStyle: "transform: translate(-50%, -50%) translate(95px, 165px);",
            labelStyle: "transform: translate(0%, -50%) translate(215px, 95px); text-align: left;",
            link: "https://энергияплюс.рф/page/proektirovanie-avtonomnyh-kotelnyh",
            arrowDeg: 195,
        },
        {
            ...
            link: "https://энергияплюс.рф/page/pusko-naladochnye-raboty",
            arrowDeg: 255,
        },
        {
...
<svelte:head>
    ...
    <meta name="description"
          content="Инжиниринг полного цикла: проектирование котельных, строительство газопроводов, блочно-модульные котельные под ключ. 22 года работы, 225 заказчиков."/>
    <meta name="keywords"
          content="котельные, проектирование котельных, строительство котельных, газоснабжение, блочно-модульные котельные, ГРПШ, газопровод"/>
    ...
    <meta property="og:description"
          content="Инжиниринг полного цикла: проектирование, строительство, сервис котельных и газопроводов. 22 года работы."/>
...
<div id="ep-root">
    ...
    <section class="hero-section" aria-label="Главный баннер">
        <div class="video-box">
            <div class="hero-head">
                ...
                <a href="https://xn--c1adkmgpem4hrai.xn--p1ai/uslugi" target="_blank" rel="noopener"
                   class="video-top-bar__link">Перечень выполняемых работ</a>
            </div>
            <video
                    bind:this={heroEl}
                    class="video-poster"
            ...
            <div class="top-services-grid">
                {#each services as svc}
                    <a href={svc.href} target="_blank" rel="noopener" class="top-service-card" aria-label={svc.title}>
                        <img loading="lazy" src={svc.img} alt={svc.alt} class="top-service-img" width="400"
                             height="533"/>
                ...
                <div class="scheme-wrap">
                    <div class="scheme-system" role="img" aria-label="Схема инжинирингового цикла">
                        <svg style="position:absolute;top:0;left:0;width:100%;height:100%;z-index:1;"
                             viewBox="0 0 580 580" aria-hidden="true">
                        ...
                        <div class="center-circle">
                            <img src="/ickonandlogo/newsrtikon/worker.svg" alt="Инженер" class="center-icon" width="270"
                                 height="270"/>
                        </div>
                        ...
                        {#each schemeItems as item, idx}
                            <div class="sat-circle" style={item.satStyle}>
                                {#if idx === 0}
                                    <svg class="sat-icon sat-icon--doc" viewBox="148 148 478 478"
                                         xmlns="http://www.w3.org/2000/svg" fill="white" aria-hidden="true">
                                        <defs>
                                ...
                                {:else}
                                    <img src={item.svgSrc} alt="" class="sat-icon" width="50" height="50"
                                         aria-hidden="true"/>
                                {/if}
                            </div>
                    ...
                    <ul class="eng-list">
                        {#each advantages as adv}
                            <li>
                                ...
                                {:else}
                                    <img src={adv.svgIcon} alt="" class="adv-icon" width="22" height="22"
                                         aria-hidden="true"/>
                                {/if}
                                {adv.text}
                            </li>
                ...
                {#each stats as s}
                    <div class="stats-card">
                        <div class="stats-number">
                            {s.number}
                            {#if s.unit}<span class="stats-unit">{s.unit}</span>{/if}
                        </div>
                        ...
                        <div class="booklet-img bmk-video-box">
                            ...
                            <div class="video-controls">
                                ...
                                <div class="vc-row">
                                    ...
                                    <input class="vc-volume" type="range" min="0" max="1" step="0.05" value={bmkVolume}
                                           on:input={bmkChangeVolume}/>
                                    <div class="vc-speed-wrap">
                                        <button class="vc-btn vc-speed-btn"
                                                on:click={() => (bmkSpeedMenu = !bmkSpeedMenu)}>{bmkSpeed}x
                                        </button>
                                        {#if bmkSpeedMenu}
                                            <div class="vc-speed-menu">
                                                {#each speeds as s}
                                                    <button class="vc-speed-option {s === bmkSpeed ? 'active' : ''}"
                                                            on:click={() => bmkSetSpeed(s)}>{s}x
                                                    </button>
                                                {/each}
                                            </div>
                                        {/if}
                        ...
                        <div class="booklet-img">
                            {#if b.href}
                                <a href={b.href} target="_blank" rel="noopener"
                                   aria-label="Открыть буклет: {b.heading}">
    ...
    {#if showModal}
        ...
        <div class="order-modal-overlay" on:click|self={() => (showModal = false)} role="dialog" aria-modal="true"
             aria-label="Форма заказа котельной">
            <div class="order-modal">
                <button class="order-modal__close" on:click={() => (showModal = false)} aria-label="Закрыть">&times;
                </button>
                ...
                <form class="order-modal__form" on:submit={handleFormSubmit}>
                    ...
                    <div class="order-modal__field">
                        <textarea name="message" placeholder="Расскажите о вашем проекте"
                                  class="order-modal__input order-modal__textarea"></textarea>
                    </div>
...
<style>
    ...
    #ep-root .vc-unmute-hint {
        ...
        font-weight: 600;
        color: #fff;
        background: rgba(240, 112, 48, 0.85);
        border-radius: 4px;
        padding: 2px 7px;
        white-space: nowrap;
    ...
    #ep-root .bmk-video-box:hover .play-btn {
        opacity: 1;
    }
    ...
    #ep-root .bmk-video-box:hover .video-controls {
        opacity: 1;
    }

    #ep-root .vc-progress-wrap {
        width: 100%;
        padding: 4px 0;
    }
    ...
    #ep-root .vc-seek::-webkit-slider-thumb:hover {
        transform: scale(1.3);
    }
    ...
    #ep-root .vc-speed-option:hover {
        background: rgba(255, 255, 255, 0.1);
    }

    #ep-root .vc-speed-option.active {
        color: #f07030;
        font-weight: 700;
    }
    ...
    #ep-root .top-service-card:hover .top-service-img {
    ...
    #ep-root .top-service-card:hover .top-service-overlay {
    ...
    #ep-root .booklet-block:last-child {
        border-bottom: none;
    }
    ...
    #ep-root .booklet-img:hover {
        transform: translateY(-4px);
        box-shadow: 0 16px 40px rgba(211, 93, 45, 0.2);
    }

    #ep-root .booklet-img img {
        width: 100%;
        height: 100%;
        object-fit: cover;
        display: block;
    ...
    #ep-root .bmk-video-box {
        position: relative;
        overflow: hidden;
        background: #111;
    }

    #ep-root .booklet-content {
        flex: 1;
    }
    ...
    #ep-root .booklet-heading--link:hover {
        color: #f07030;
    ...
    #ep-root .order-modal__close:hover {
        color: #d35d2d;
    }
    ...
    #ep-root .order-modal__input:focus {
        border-color: #f07030;
    }

    #ep-root .order-modal__input::placeholder {
        color: #bbb;
    }

    #ep-root .order-modal__textarea {
        resize: vertical;
        min-height: 100px;
    }
    ...
    #ep-root .order-modal__submit:hover {
        background: #d35d2d;
        transform: translateY(-2px);
    }
</style>